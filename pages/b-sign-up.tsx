...
export default function SignUp() {
  ...
  const checkId = async () => {
    try {
      const response = await axios.get(`http://192.168.0.112:9988/api/check-id?id=${id}`);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // 회원가입 요청
    try {
      const response = await axios.post("http://192.168.0.112:9988/api/b-signup", {
        id,
        password,
        name,
        brn,
  ...
  return (
    <div className="mx-auto max-w-[350px] space-y-6">
      <div className="space-y-2 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400">취지직은 사업자 여러분을 환영합니다.</p>
      </div>
      <div>
